// $Header: /repository/PI_annex/robsandbox/KoMer/apps/FilterReads.cpp,v 1.5 2010-02-22 14:41:37 regan Exp $
//
...
{
public:
    static int getMaxKmerDepth() { return getVarMap()["max-kmer-depth"].as< unsigned int >(); }
    static int getPartitionByDepth() { return getVarMap()["partition-by-depth"].as< unsigned int > (); }
	static bool parseOpts(int argc, char *argv[])
	{
        ...
        getDesc().add_options()
        ("max-kmer-depth",     po::value< unsigned int >()->default_value(0), "maximum number of times a kmer will be represented among the selected reads (mutually exclusive with partition-by-depth)")
        ("partition-by-depth", po::value< unsigned int >()->default_value(1), "partition filtered reads by powers-of-two coverage depth (mutually exclusive with max-kmer-depth)" )
        ;
   ...
   if (!FilterReadsOptions::parseOpts(argc,argv))
    ...
    if (maximumKmerDepth > 0) {
    	for(int depth = 1 ; depth < maximumKmerDepth ; depth++) {
    		...
    		if (reads.hasPairs())
    		    picked += selector.pickBestCoveringSubsetPairs(depth, Options::getMinDepth(), Options::getMinReadLength());
    		else
    		    picked += selector.pickBestCoveringSubsetReads(depth, Options::getMinDepth(), Options::getMinReadLength());
    	}
	    ...
	    for(unsigned int depth = maxDepth ; depth >= 1; depth /= 2) {
	      if (reads.hasPairs())
	        picked = selector.pickAllPassingPairs(depth, Options::getMinReadLength());
	      else
	        picked = selector.pickAllPassingReads(depth, Options::getMinReadLength());
